...
from rasp.msg import ik_joints
import numpy as np
...
# with open('trial_mpc.csv', 'r') as csvfile:
...
#     target_pos=[data.x,data.y,data.z,data.phi]
    target_pos=[data.x,data.y,data.z,-0.4]

    # target_pos = [data.data, 0.08, 0.15, 0]
    #target_pos = [data.data, as2, as3, 0]
    omx_inverse_kinematics(target_pos)

# Create Publisher and Subscriber Node
rospy.Subscriber("custom_chatter",ik_joints,topic_callback)
#rospy.Subscriber("servo1",Float64,topic_callback)
joint_angles_pub = rospy.Publisher("Output_joint_position", Float64, queue_size=10)
...
# def omx_inverse_kinematics():
    # target_pos = [0.22, 0.08, 0.18, 0]


    # target_pos = [as1, as2, as3, 0]
    # End effector pose
